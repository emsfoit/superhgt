...
'''
    Dataset arguments
'''
parser.add_argument('--data_dir', type=str, default='dataset/oag_output/without_extra_links/50000',
...
logger('Number of nodes In the graph:')
for node_type in graph.node_forward:
    logger(f'{node_type}: {len(graph.node_forward[node_type]):,}')
...
if use_last_model:
    filename = os.path.join(args.model_dir, 'last_model_' + args.task_name + '_' + args.conv_name)
    logger("=> loading checkpoint '{}'".format(filename))
    checkpoint = torch.load(filename)
    start_epoch = checkpoint['epoch']
    best_val = checkpoint['best_val']
    loaded_model = checkpoint['model']
    gnn, classifier = loaded_model
    gnn = gnn.to(device)
    classifier = classifier.to(device)
    model = nn.Sequential(gnn, classifier)
    logger("loaded best_val = {}".format(best_val))
else: 
...
i = epoch = 0
for mio in np.arange(args.n_epoch) + 1:
    epoch = mio+start_epoch
    '''
    ...
    for _ in range(args.repeat):
        for node_feature, node_type, edge_time, edge_index, edge_type, x_ids, ylabel in train_data:
            ...
            if i == 0:
                i+=1
                # tb.add_graph(gnn, (node_feature.to(device), node_type.to(device), edge_time.to(device), edge_index.to(device), edge_type.to(device)) )
                tb.add_graph(classifier, node_rep[x_ids])
            ...
            del res, loss
    tb.add_scalar('training loss', np.average(train_losses), epoch)
    tb.add_histogram('classifier.linear.bias', classifier.linear.bias, epoch)
    ...
    with torch.no_grad():
        ...
        tb.add_scalar('NDCG', valid_ndcg, epoch)
        valid_mrr = np.average(mean_reciprocal_rank(valid_res))
        tb.add_scalar('MRR', valid_mrr, epoch)
        
        if valid_ndcg > best_val:
            best_val = valid_ndcg
            state = {'epoch': epoch + 1, 'model': model, 'best_val': best_val }
            torch.save(state, os.path.join(args.model_dir, args.task_name + '_' + args.conv_name))
            logger('UPDATE!!!')
        ...
        tb.add_scalar('valid loss', loss.cpu().detach().tolist(), epoch)
        tb.add_histogram('classifier.linear.bias', classifier.linear.bias, epoch)
        del res, loss
    del train_data, valid_data

state = {'epoch': epoch, 'model': model, 'best_val': best_val }
torch.save(state, os.path.join(args.model_dir, 'last_model_' + args.task_name + '_' + args.conv_name))
...
filename = os.path.join(args.model_dir, args.task_name + '_' + args.conv_name)
logger("=> Evaluating")
checkpoint = torch.load(filename)
...
end_time = time.time()
logger('%.1fs' % (end_time-start_time))
